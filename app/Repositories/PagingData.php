...
class PagingData
{
    ...
    public function validateRequest()
    {
        ...
        // for custom messages
        $messages = [
            'sort.name.in' => 'Only Allowed: ' . implode(', ', $orderableColumns),
            'sort.dir.in' => 'Only allowed \'asc\', \'desc\' and \'random\'',
        ];
    ...
    public function paginateData(QueryBuilder $query, $where = [])
    {
        ...
        // ordering data
        $sortBy = !empty($validated['sort']['name']) ? $validated['sort']['name'] : null;
        if (!empty($sortBy)) {
            $sortDir = $validated['sort']['dir'];
            if ('random' == $sortDir) {
                $query->inRandomOrder();
            } else {
